...
public class PigChase : MonoBehaviour
{
    ...
    private void Chasing()
{
    ...
    if (m_playerInRange)
    {
        // If the enemy sees the player, stop patrolling and chase the player
        m_IsPatrol = false;
        m_PlayerNear = false;
        Move(speedRun);
    ...
    if (navMeshAgent.remainingDistance <= navMeshAgent.stoppingDistance)
    {
        ...
        {
            // Check if the enemy is not near the player, returns to patrol after the wait time delay
            m_IsPatrol = true;
            m_PlayerNear = false;
            Move(speedWalk);
            m_TimeToRotate = timeToRotate;
...
void EnviromentView()
{
    Collider[] playerInRange = Physics.OverlapSphere(transform.position, viewRadius, playerMask); // Make an overlap sphere around the enemy to detect the player mask in the view radius
    ...
    for (int i = 0; i < playerInRange.Length; i++)
    {
        ...
        if (angleToPlayer < viewAngle / 2 && angleToPlayer >= -viewAngle / 2)
        {
            ...
            if (!Physics.Raycast(transform.position, dirToPlayer, dstToPlayer, obstacleMask))
            {
                ...
                // The player has been seen by the enemy, and then the enemy starts to chase the player
        ...
        if (Vector3.Distance(transform.position, player.position) > viewRadius)
        {
            ...
            // Or if the enemy is in a safe zone, the enemy will not chase
            m_playerInRange = false;
        }
    ...
    // private void Update()
    // {
    //     // Check if the playerTransform is not null
    //       Debug.Log("Updating");
