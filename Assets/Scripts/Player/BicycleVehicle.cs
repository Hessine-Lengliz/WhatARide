...
public class BicycleVehicle : MonoBehaviour
{
	...
	public float minSpeed = 0f; // Minimum animation speed
	public float maxSpeed = 10f; // Maximum animation speed
...
private bool isSoundPlayed = false;
    ...
    Transform characterTransform = transform.Find("happy wheel character riding");
    ...
    float bikeSpeed = rb.velocity.magnitude;
    float animationSpeedMultiplier = Mathf.Clamp(bikeSpeed / 10f, 0.5f, 2f);

    // Add a threshold for stopping animation
    if (bikeSpeed < 0.1f)
    {
        animator.speed = 0f;

        // Reset the flag when the bike comes to a stop
        isSoundPlayed = false;
    }
    else
    {
        ...
        // Check if the sound is not already playing and hasn't been played yet
        if (!bikeSound.isPlaying && !isSoundPlayed)
        {
            bikeSound.Play();
            isSoundPlayed = true;
        }
